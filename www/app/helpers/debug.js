...
function echo() {
    ...
    var replacer = function(s, m1, m2) {
        ...
        // Doesn't cover all cases yet: see http://php.net/manual/en/language.types.string.phplanguage.types.string.syntax.double
        if (m1 !== '\\') {
            return m1 + eval(m2);
        } else {
            return s;
        ...
        if (d.appendChild) {
            if (d.body) {
                if (win.navigator.appName === 'Microsoft Internet Explorer') {// We unfortunately cannot use feature detection, since this is an IE bug with cloneNode nodes being appended
                    ...
                    // We will not actually append the div tag (just using for providing XHTML namespace by default)
                    if (unappendedLeft) {
